...
import { FiBriefcase, FiGithub, FiGlobe, FiLinkedin, FiMapPin, FiTwitter } from 'react-icons/fi'
...
const Profile = () => {
  ...
  return (
    <div className='w-screen h-screen flex flex-col items-center justify-center bg-gray-500 relative'>
      ...
      <div className='w-90 md:w-800 h-auto md:h-500 flex flex-col bg-white relative rounded-md border-thin border-slate-200 mt-20'>
        ...
        <div className='w-full flex flex-col md:flex-row items-center justify-between px-4 py-2 mt-28 md:mt-4'>
          <div className='flex items-center gap-3'>
            <div className='flex flex-col items-center justify-center'>
              <p className='text-lg font-bold'>{DUMMY_USER.posts.length}</p>
              <p>Posts</p>
            </div>
            <div className='flex flex-col items-center justify-center'>
              <p className='text-lg font-bold'>{DUMMY_USER.followers.length}</p>
              <p>Followers</p>
            </div>
            <div className='flex flex-col items-center justify-center'>
              <p className='text-lg font-bold'>{DUMMY_USER.following.length}</p>
              <p>Following</p>
            </div>
          </div>
        ...
        <div className='w-full flex flex-col items-center mt-0 md:mt-8'>
          ...
          <div className='flex items-center gap-4 mt-4'>
            <a href={DUMMY_USER.url} target='_blank' rel='noopener noreferrer' className='text-slate-500 text-2xl'><FiGlobe /></a>
            <a href={DUMMY_USER.github} target='_blank' rel='noopener noreferrer' className='text-slate-500 text-2xl'><FiGithub /></a>
            <a href={DUMMY_USER.linkedin} target='_blank' rel='noopener noreferrer' className='text-slate-500 text-2xl'><FiLinkedin /></a>
            <a href={DUMMY_USER.twitter} target='_blank' rel='noopener noreferrer' className='text-slate-500 text-2xl'><FiTwitter /></a>
          </div>
          <div className='w-full flex flex-col items-center border-t-thin border-slate-100 mt-12 pt-4'>
            <p className='w-4/5 md:w-2/3 text-center text-slate-500'>{DUMMY_USER.bio}</p>
          </div>
          <div className='w-full flex items-center justify-center gap-4 mt-4'>
            <div onClick={() => switchTabs('posts')} className={`px-4 py-2 cursor-pointer ${activeTab.posts ? 'bg-slate-500 text-white' : 'bg-white text-slate-500'}`}>
              Posts
            </div>
            <div onClick={() => switchTabs('followers')} className={`px-4 py-2 cursor-pointer ${activeTab.followers ? 'bg-slate-500 text-white' : 'bg-white text-slate-500'}`}>
              Followers
            </div>
            <div onClick={() => switchTabs('following')} className={`px-4 py-2 cursor-pointer ${activeTab.following ? 'bg-slate-500 text-white' : 'bg-white text-slate-500'}`}>
              Following
            </div>
          </div>
