...
const PostForm = () => {
  ...
  return (
    ...
    <form onSubmit={handleCreatePost} className='w-90 border-thin border-slate-400 rounded-lg mb-6 p-4'>
        <textarea name='message' value={message} onChange={(e) => setMessage(e.target.value)} placeholder='Create a post...' className='focus:outline-none w-full rounded-lg p-2 text-sm bg-slate-400 text-slate-900 dark:text-white border border-transparent appearance-none rounded-tg placeholder-gray-400'></textarea>
        <footer className='w-full flex items-center justify-between mt-2'>
            <div className='flex gap-2'>
                ...
                <div className='w-12 h-12 user-select-none'><img src={previewURL} className='w-full h-full object-cover' /></div>
            </div>
